...
(function(factory) {
    ...
    defaults = {
        ...
        //show or hide country flag, could be "full" (normal), "single" (no dropdown/arrow), or "none" (no selected country or dropdown - just a normal input)
        countryMode: "full",
        // default country
        defaultCountry: "",
   ...
   ********************/
        ...
        _generateMarkup: function() {
            ...
            this.telInput.wrap($("<div>", {
                "class": "intl-tel-input " + this.options.countryMode
            }));
            if (this.options.countryMode !== "none") {
                ...
                if (this.options.countryMode === "single") {
                    delete flagAttributes.tabindex;
                }
                selectedFlag = $("<div>", flagAttributes).appendTo(this.flagsContainer);
                this.selectedFlagInner = $("<div>", {
                    "class": "iti-flag"
                }).appendTo(selectedFlag);
                // CSS triangle
                if (this.options.countryMode === "full") {
                    $("<div>", {
                        "class": "arrow"
                    }).appendTo(selectedFlag);
                }
        ...
        _setInitialState: function() {
            ...
            } else if (this.options.countryMode !== "none" && this.options.defaultCountry != "auto") {
        ...
        _updateFlagFromNumber: function(number, updateDefault) {
            //if in 'single' or 'none' mode, no need to update flag from the number
            if (this.options.countryMode !== "full") {
                return;
            }
   ...
   ********************/
        ...
        // validate the input val - assumes the global function isValidNumberForRegion (from utilsScript)
        isValidNumberForRegion: function() {
            var val = $.trim(this.telInput.val()), countryCode = this.options.nationalMode ? this.selectedCountryData.iso2 : "";
            if (window.intlTelInputUtils) {
                return intlTelInputUtils.isValidNumberForRegion(val, countryCode);
            }
            return false;
        },
