...
function getNationalSignificantNumber(phone,country) {
  try {
    var phoneUtil = i18n.phonenumbers.PhoneNumberUtil.getInstance();
    var number = phoneUtil.parseAndKeepRawInput(phone, country);
    if (phoneUtil.isValidNumberForRegion(number, country)) {
      var output = phoneUtil.getNationalSignificantNumber(number);
      return output;
    } else {
      return phone.replace(/[^0-9]/ig, "");
    }
  } catch (e) {
    return phone && phone.replace(/[^0-9]/ig, "");
  }
}
...
goog.exportSymbol('intlTelInputUtils.getNationalSignificantNumber', getNationalSignificantNumber);
// enums
